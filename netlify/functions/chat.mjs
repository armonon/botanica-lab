...
export async function handler(event) {
  if (event.httpMethod === 'OPTIONS') {
    return { statusCode: 204, headers: response(204, {}).headers, body: '' };
  }
  ...
  if (!process.env.OPENAI_API_KEY) {
    return response(503, { error: 'Botanica chat backend is not configured yet.' });
  }
  ...
  try {
    ...
    if (!latestUser || latestUser.content.length < 2) {
      return response(400, { error: 'Please send a formulation question or blend idea.' });
    }
    ...
    if (!openaiResponse.ok) {
      const detail = data.error?.message || `OpenAI request failed with ${openaiResponse.status}`;
      return response(openaiResponse.status, { error: detail });
    }

    const reply = data.choices?.[0]?.message?.content?.trim();
    if (!reply) {
      return response(502, { error: 'The model returned an empty response.' });
    }

    return response(200, { reply });
  } catch (error) {
    return response(500, { error: error.message || 'Unexpected Botanica chat error.' });
  }
}
